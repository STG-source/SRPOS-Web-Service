...
function getCustomerByRange($id){
	try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "select * from `_ccrr_base` where `calendarID` = " . $id;
    $handle = mysql_query($sql);
    //echo $sql;
    $row = mysql_fetch_object($handle);
	$clsCustomer = new ScustomerService();
	return $clsCustomer->get_customerByCustomerID($row->customerID);
...
if($_GET["id"]){
  ...
  $customer = getCustomerByRange($_GET["id"]);
}
?>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("#Savebtn").click(function() {
				var msg = "";
				if($("#Name").val() == ""){
					msg = msg + "Please Insert Customer Name !!\n";
				}
				
				if($("#CitizenID").val() == ""){
					msg = msg + "Please Insert CitizenID !!\n";
				}
				
				if($("#Telephone").val() == ""){
					msg = msg + "Please Insert Telephone No. !!\n";
				}
      ...
      <div class="infocontainer">            
        <form action="php/datafeed.php?method=adddetails<?php echo isset($event)?"&id=".$event->Id:""; ?>" class="fform" id="fmEdit" method="post">                 
          ...
          <option value="" selected="selected">-- Select --</option>
            ...
            <option value="R4-1">R4-1</option>
            <option value="R4-2">R4-2</option>
          ...
          <input name="Name" id="Name" type="text"  style="width:85%;" value="<?php echo isset($customer)?$customer->fullname:""; ?>"  />
             <span>
          CitizenID
          </span>     
          <input name="CitizenID" id="CitizenID" type="text"  style="width:85%;" value="<?php echo isset($customer)?$customer->citizenID:""; ?>"  />
          <span>
          Telephone
          </span>     
          <input name="Telephone" id="Telephone" type="text"  style="width:85%;" value="<?php echo isset($customer)?$customer->cellPhone:""; ?>"  />
          <span>
          Email
          </span>     
          <input name="Email" id="Email" type="text"  style="width:85%;" value="<?php echo isset($customer)?$customer->email:""; ?>"  />
            <span>                        Remark:
            </span>                    
